...
from open3d import ScalableTSDFVolume
...
def register_one_rgbd_pair(s, t, color_files, depth_files, intrinsic,with_opencv, config):
    ...
    if abs(s - t) is not 1:
        if with_opencv:
            ...
            if success_5pt:
                [success, trans, info] = compute_rgbd_odometry(source_rgbd_image, target_rgbd_image, intrinsic, odo_init,RGBDOdometryJacobianFromHybridTerm(), option)
    ...
    for s in range(sid, eid):
        for t in range(s + 1, eid):
            ...
            if t == s + 1:
                ...
                [success, trans,
                 info] = register_one_rgbd_pair(s, t, color_files, depth_files,intrinsic, with_opencv, config)
                trans_odometry = np.dot(trans, trans_odometry)
                trans_odometry_inv = np.linalg.inv(trans_odometry)
                pose_graph.nodes.append(PoseGraphNode(trans_odometry_inv))
                pose_graph.edges.append(PoseGraphEdge(s - sid,t - sid,trans,info,uncertain=False))
            ...
            if s % config['n_keyframes_per_n_frame'] == 0 \
                ...
                if success:
                    pose_graph.edges.append(o3d.registration.PoseGraphEdge(s - sid,t - sid,trans,info,uncertain=True))
    write_pose_graph( join(path_dataset, config["template_fragment_posegraph"] % fragment_id),pose_graph)
...
def process_single_fragment(fragment_id, color_files, depth_files, n_files,n_fragments, config):
    if config["path_intrinsic"]:
        intrinsic = read_pinhole_camera_intrinsic("camera_real.json")
    else:
        intrinsic = PinholeCameraIntrinsic(era.PinholeCameraIntrinsicParameters.PrimeSenseDefault)
...
def run(config):
    ...
    if config["python_multi_threading"]:
        ...
        Parallel(n_jobs=MAX_THREAD)(delayed(process_single_fragment)(fragment_id, color_files, depth_files, n_files, n_fragments, config) for fragment_id in range(n_fragments))
    else:
